...
export const updateUserProfile = async (
  ...
  const updatedUser: User = {
    ...
    viviendaId: data.viviendaId !== undefined ? data.viviendaId : existing.viviendaId,
  ...
  try {
    ...
    await setDoc(
      userDocRef,
      {
        ...
        ...(updatedUser.viviendaId ? { viviendaId: updatedUser.viviendaId } : {}),
...
/**
 * Asocia o desvincula a un usuario de una vivienda
 */
export const setUserDwelling = async (userId: string, viviendaId: string | null): Promise<void> => {
  if (!userId) return;
  const current = getLocalProfile(userId) || createDefaultProfile(userId);
  const updated: User = {
    ...current,
    viviendaId: viviendaId || undefined,
    updatedAt: new Date().toISOString(),
  };
  ...
  try {
    ...
    console.warn("Aviso al actualizar vivienda en Firestore:", err);
  }
};

/**
 * Obtiene perfiles de múltiples usuarios según sus IDs
 */
